...
export default function ParentFinancePage() {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      setPageLoading(true);
      setErrorMsg(null);
      const res = await api.get(`/finance/parents/students/${studentId}/fees`);
      setInvoices(res.data.data || res.data);
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to fetch child invoices');
    } finally {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    setSuccessMsg(null);
    try {
      await api.post(`/finance/parents/students/${studentId}/fees/${selectedInvoice.id}/pay`, {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between">
        <div>
          <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider block">Guardian portal</span>
      ...
      {errorMsg && (
        <div className="p-4 rounded-xl border border-red-500/25 bg-red-500/5 text-red-400 flex items-start gap-3 text-sm">
          <AlertTriangle className="w-5 h-5 shrink-0" />
          <div>{errorMsg}</div>
        </div>
      )}
      {successMsg && (
        <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 flex items-start gap-3 text-sm">
      ...
      <div className="bg-slate-900/30 border border-white/10 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <h2 className="font-bold text-sm tracking-wide">Tuition Statements</h2>
          <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-300">
              ...
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 whitespace-nowrap">{inv.academicTerm}</td>
                    <td className="p-4 whitespace-nowrap text-slate-450">{inv.dueDate}</td>
                    <td className="p-4 whitespace-nowrap text-center">
                      <span className={`inline-flex px-2 py-0.5 border text-[9px] font-extrabold uppercase tracking-wide ${
                        inv.status === 'PAID'
                    ...
                    <td className="p-4 text-center whitespace-nowrap">
                      {balance > 0 ? (
                        ...
                        <span className="text-emerald-400 font-bold inline-flex items-center gap-0.5"><CheckCircle className="w-3.5 h-3.5" /> Cleared</span>
                      )}
                    </td>
                  </tr>
                );
              ...
              {invoices.length === 0 && (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-500">
                    No active invoices issued for your child yet.
                  </td>
                </tr>
              )}
            </tbody>
