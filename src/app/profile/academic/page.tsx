...
export default function StudentViewOnlyAcademicDashboard() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      setLoading(true);
      try {
        const res = await api.get('/academic/students/me/details');
        setData(res.data.data || res.data);
      } catch (err: any) {
        console.error(err);
        setError(err.response?.data?.message || 'Failed to retrieve your academic performance profile.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-400 text-sm font-semibold tracking-wider uppercase">Loading your academic dossier...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center space-y-4 text-slate-100">
        ...
        <p className="text-rose-400 text-sm font-bold text-center max-w-md px-6">{error || 'Academic profile details could not be found.'}</p>
        <button 
          onClick={() => router.push('/profile')}
          className="text-xs font-semibold uppercase bg-slate-900 border border-white/5 px-4 py-2.5 rounded-xl text-slate-300 hover:text-white transition-all cursor-pointer"
        >
            ...
            <div className="space-y-4">
              {quizzes.length === 0 ? (
                <div className="text-center py-16 border border-dashed border-white/5 rounded-3xl text-sm text-slate-500">
                  ...
                  No quiz attempts logged for your profile.
                </div>
              ) : (
                                          ...
                                          return (
                                            <div key={qidx} className="bg-slate-900/40 border border-white/5 rounded-2xl p-4 space-y-2">
                                              ...
                                              <div className="text-[10px] pt-1.5 flex items-center justify-between text-slate-500 font-mono">
                                                <span>Your response: <strong className={isCorrect ? "text-emerald-400" : "text-rose-400"}>{studentAns || 'Skipped'}</strong></span>
                      ...
                      <div className="bg-slate-900/30 border border-white/5 rounded-2xl p-6 space-y-4 shadow-xl">
                        ...
                        <div className="space-y-2 text-xs pt-2">
                          ...
                          <div className="bg-purple-900/5 p-4 border border-purple-500/10 rounded-xl space-y-1.5">
                            <span className="text-[9px] text-purple-400 uppercase font-bold tracking-wider">Performance Feedback</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-slate-900/20 border border-white/5 rounded-2xl p-6 md:p-8 space-y-6 shadow-xl">
                ...
                <div className="grid grid-cols-1 gap-5 text-xs">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block">Your Student Reg No</span>
                ...
                <div className="space-y-6">
                  {student.parents.length === 0 ? (
                    <div className="text-center py-12 text-xs text-slate-500">
                      No linked parent/guardian records found.
                    </div>
                  ) : (
